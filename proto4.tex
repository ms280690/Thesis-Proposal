...
The aim of this prototype is to embed \languageConstruct{IO} operations within the definition on an eDSL so as to allow the chaining and control 
...
\progLang{Haskell} calls itself a pure functional programming language. Every function in Haskell is a function in the mathematical sense (i.e., ``pure'').
...
\end{minted}
\end{singlespace}
\caption{Example operation using \haskellClass{do} notation}
...
Listing~\ref{tab:edslpureimpure} describes a micro language having constructors for pure and impure actions. The \haskellConstruct{Pure a}
constructor represents a result that is returned while \haskellConstruct{Impure (f (Term f a))} build a compound \haskellConstruct{Term}s
...
operations. The \haskellClass{OneBinding} constructor represents a \prologConstruct{unifier} as a pairs of variables. For termination we use
\haskellClass{NoResult}. 
...
>>>>>>> added, removed polished content in section 13.4 in proto4
\end{code-list}
