...
\item Interpreter which consists of a parsing mechanism and resembles the query resolver.\endnote{%
  The first two items start with ``\textit{The}''.  The remaining items
  do not, which is probably incorrect.
}
\end{enumerate}
...
\item Variables\endnote{%
  what do ``\textit{Variables}'' refer to?
},
...
The possibilities are,\endnote{%
  I believe that I have commented on the preceding elsewhere.
  The punctuation to introduce a list (that is, the end of the phrase immediately preceding the list) should not be
  ``,'', but more likely a colon, or possibly a period.
  There should also be punctuation after each item in a list.
  See item~\ref{item:5} in the to-do list.
}\elabel{comma-before-list}
\begin{enumerate}
...
unified i.e searching.\endnote{%
  ``i.e.'' is missing the final period.
  Secondly, ``\textit{where two terms need to be
    unified that is searching}'' doesn't read well because it is missing
  commas.  Check that latin abbreviations read correctly when replaced
  with English words.
}
...
\end{minted}
%
\yyy{}{whose operation is illustrated in Figure~\ref{fig:Trace for append}.}
In this prototype we explore the unification aspect only.\endnote{%
  I've tidied up a bit here.  See if this reads more clearly.  I've also
  removed a comment.\newline  (Commit \textsf{\bfseries fe997de\-08bda2\-64c453\-204e97\-1a1cfd\-219457acf}).
}
...
This prototype throws light on the process of tackling the issues involved in creating a data type to replicate the
target language type system while conforming to the host language restrictions and also utilizing the benefits.
...
To start we need to define a abstract syntax for the \progLang{Prolog} like\eref{language-like} language.
But there is a conflict between the type systems as we shall discuss.


A type system consists of a set of rules to define a \yyy{"}{``}type\yyy{"}{''} to different constructs in a
...
The advantages of static typing \cite{meijer2004static}\endnote{
  ...
  If we are going to keep this, we need to have sentences.  We need something like
  ``\textsl{The advantages of static typing are: \dots{}, and\dots, whereas the advantages of dynamic typing are:
    \dots}''.
}
\begin{enumerate}
...
Since \progLang{Haskell} is statically type we would need to define a \yyy{"}{``}typed\yyy{"}{''} language which
should have a number of constructs representing different terms in \progLang{Prolog} such as complex structures
(for example predicates, clauses etc.), don't cares, cuts, variables and so on.\endnote{% 
  Drop the ``would''s here.
}
...
Moreover, the primitives of the language\yyy{}{(see \cite{website:understandingalgebrasfpcomplete})}\endnote{%
  is this ok?
}
are not accessible as the language can have expressions of only one
type\yyy{}{\Large,} i.e.\yyy{}{\Large,} ``\unknownLabel{Term}''.
...
\end{comment}
\yyy{Although one}{One} problem remains\yyy{,}{:} how does one represent \xxx{infinitely nested /} deep expressions
of the above language, 
for example something of the form,
\mint{haskell}|FlatTerm(FlatTerm (FlatTerm (FlatTerm (....... (a)))))|
and how to represent it generically to perform operations on it\yyy{ since,}{, since}
\begin{minted}[linenos]{haskell}
...
\markWord{Term} no matter how infinitely deep they were.
...
difference.
It means that applying one more \markWord{FlatTerm} would not change anything \yyy{–}{--}\eref{non-ascii}\textsuperscript{,}\endnote{%
  Even if entered correctly, an en-dash is not appropriate here.
}
a \textcolor{red}{fix}\endnote{%
  Spelling!
}
point does not move under \markWord{FlatTerm}.
...
\item And\eref{no-and} in type constructor form,
...
\end{minted}
%
simply speaking all the expressions resulting from \markWord{FlatTerm} can be represented  by the type signature \markWord{Fix FlatTerm}.
...
\yyy{Below}{\Huge Figure~}\ref{fig:Functor Hierarchy} shows the relation between the different classes.
...
variable with language values\yyy{}{\Verb[showspaces]! !}(language constructs).

Enter\xxx{,} \Verb[formatcom=\bf]!Traversable!\yyy{,}{\,.}%}{
\yyy{it}{It} allows reduce whilst preserving the shape of the structure.
Lastly, the Applicative instance is an intermediate between a functor and a monad.
...
as shown in \yyy{Figure}{Listing}~\ref{tab:flat-term-class-inst}.
\begin{code-list}
  \begin{singlespace}
...
The \Verb!UTerm! data type captures the recursive structure of logic terms\yyy{}{\Large,} i.e\yyy{}{\Large.}, given
...
the library's operators to perform the recursive unification\xxx{, subsumption}.
...
\begin{comment}
  ...
  The IntVar implementation uses Int as the names of variables, and uses an IntMap to keep track of the
  environment.
  The STVar implementation uses STRefs, so we can use actual mutation for binding logic variables, rather than
  keeping an explicit environment around.
  This implementation uses STVars.
  ...
  The BindingMonad type class provides the definition of what we need from our ambient monad.
  ...
  The library provides the necessary instances for both IntVar and STVar.
