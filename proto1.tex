...
\section{Components}
There are four main components\endnote{%
...
\item
  And finally the query resolver which combines the unification and search strategy to return a result.\endnote{%
...
\begin{enumerate}
\item \yyy{}{the} \yyy{L}{l}anguage, adopted from \progLang{Prolog} but trimmed down\yyy{.}{;}

\item \yyy{D}{the d}atabase\yyy{}{;}\endnote{%
  and so on.
}
...
The possibilities \yyy{could be}{are},
\begin{enumerate}
...
\item If \mSV{term1} is \yyy{a}{an uninstantiated} variable and \mSV{term2} is any type of term, then \mSV{term1}
