...
\item \codeLibrary{prolog-0.2.0.1} \cite{prolog-lib}
  ...
  certain components to test\endnote{%
    Do you mean ``test''?
    \mehul{Yes}
}
  our approach.
...
Basically we do Unification\eref{caps-choices}
monadically.
...
Since \progLang{Haskell} is statically type we would need to define a ``typed\yyy'' language which
...
\begin{comment}
The \xxx{above} data type Figure~\ref{tab:closed-terms} is recursive as seen in the constructor,
...
\david{\macroName{codeLibrary} is local to this file, which creates a
  problem in the Table of Contents.}

Our language is now opened up and \endnote{%
...
\begin{enumerate}
\item Functor Hierarchy.\eref{caps-choices}

\item Required instances the language must have.

\item Mutable variables.

\item Variable Bindings.\eref{caps-choices}

\item Monadic Unification.\eref{caps-choices}
