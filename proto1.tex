...
\haskellConstruct{FlatTerm} is separate from \yyy{its type}{the type of its subterms}\endnote{%
  ``\textit{its type is \haskellConstruct{a}}'' seems wrong.
  The type \haskellConstruct{a} seems associated with terms.
  The type \haskellConstruct{FlatTerm} has kind \texttt{* -> *}\,.
  \par
  Is my correction ok?
}
...
type at all.\endnote{%
  Was
  ``which is \macroName{haskellConstruct}{a}.
  The above language can be of any type \macroName{textit}{a}''
}
A more accurate way of saying it would be that \yyy{\textit{a}}{\haskellVar{a}} can be any \textit{kind} in
\progLang{Haskell}.
...
\yyy{for example \cite{website:kindhaskellwiki}}{(see \cite{website:kindhaskellwiki})}.\endnote{%
    I did not understand the ``\textit{for example\dots}''; is this right?
}
...
a\endnote{%
  Note the correct use of an em-dash.
} fix{\Large ed} point does not move under \haskellConstruct{FlatTerm}.

\progLang{Haskell} provides fixed-points in two forms, one for data and one for types.\endnote{%
  Was ``\textit{\progLang{Haskell} provides it in two forms, as a function and as a constructor.}''
  Is  ``\textsl{\progLang{Haskell} provides fixed-points in two forms, one for data and one for types.}''.
  Do you agree with the change?
  If so, remove this footnote.
}
...
\xxx{multiple} custom functionality.
\yyy{}{For instance, using \haskellConstruct{Extended} from Listing~\ref{tab:garfunkle} we have that
  \haskellConstruct{Extended} \haskellConstruct{FlatTerm} is isomorphic to the type defined in Listing~\ref{tab:man-enhan-gram}.}\endnote{%
  I've tried to guess your sense here.
  Something is mangled in the following \P{}, so i've coloured it for now..
}
...
\mint{haskell}|Extended FlatTerm| is isomorphically equivalent to the abstract grammar in Figure~\ref{fig:manual-extension}.
\end{scope}
...
The \Verb!Functor! and \Verb!Foldable! instances providing functions for applying map-reduce as described in Chapter~\ref{chap:relatedWork}.\endnote{%
  ``\textit{The \Verb!Functor! and \Verb!Foldable! instances providing functions for applying map-reduce as
    described in Chapter~\ref{chap:relatedWork}.}'' is not a sentence.
...
Lastly, the \haskellConstruct{Applicative} instance is an intermediate between a functor and a monad.
...
\begin{code-list}[H]
  \begin{singlespace}
    \inputminted[linenos,gobble=4,lastline=12]{haskell}{haskell-proto1-class-flat.hs}
  \end{singlespace}
...
implementations exist,\endnote{%
  ...
  Check to see that it is correct, or reverse commit \texttt{95804e3\dots}.
	\mehul{looks ok, cannot remember exactly}
}
...
runST is pure \cite{website:stmonadwiki, website:stmonadstackoverflow}. This is the idea behind \haskellConstruct{STVars} which are 
