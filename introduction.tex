...
Each of them has its own advantages and flaws\endnote{%
  The first character of flaws was \\u\(+\)fb02 (an FL-ligature), which makes me think
  that this text was cut and pasted.
...
choose.
The task at hand involves replicating \progLang{Prolog}-like\eref{language-like} features in \progLang{Haskell} such as unification and
...
%=======
choose. The task at hand involves replicating \progLang{Prolog} like features in \progLang{Haskell} such as unification and a single
typed system.\endnote{%
  Say more here?
  This seems like a natural place to have more high-level comments on where the thesis is going.
} The thesis aims at leveraging the features of the host language in order to incorporate logic programming features resulting in extending 
\progLang{Haskell} with capabilities like unification. We achieve this by adopting various aspects from approaches related to merging 
paradigms and embedding techniques for programming languages. This results in a hybrid approach which provides a library taking advantage 
of the host language features.
...
hassle of jumping between the two. Commonly this results in an embedded language or a foreign function interface.\endnote{%
...
The adoption of \progLang{Prolog} has been difficult since procedural languages were making a mark when it was released.
...
Putting all of the above together, Domain Specific Languages\endnote{%
  \david{I have the feeling that you should introduce the concept of DSLs
  slightly higher.}
  \mehul{higher ???}
  \david{Earlier.}
  \mehul{Moved up a few paragraphs}
}
...
master key.
A general purpose language, as the name suggests,\endnote{%
  Where I have put a {\ttfamily\expandafter\string\csname Large\endcsname} before a comma
  in a correction, it is just to make the comma more visible.
  Remove it when you do the correction.
}
...
lack the complete feature set which can make problem solving difficult.\endnote{%
  ``approaching towards the solution'' does not flow well.  Reword.
}
...
the hassle of jumping between languages or forcibly trying to solve a problem according within the limitations of a paradigm.\endnote{%
...
=======
A \progLang{Haskell} with \progLang{Prolog} like\eref{language-like} features will result in a set of characteristics which are from
>>>>>>> tagged uses of "like" that can be improved.
...
This can be achieved in two ways:\endnote{%
...
previous works and an approach for improvements.\endnote{%
