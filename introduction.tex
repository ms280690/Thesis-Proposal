...
\progLang{Haskell} with capabilities like unification. 
...
Today the number of is in the thousands and counting \cite{website:timelineproglangwiki, website:historyofproglang}.\endnote{%
...
Born in an era where procedural languages were receiving a lot of attention, it suffered\endnote{%
...
on.\endnote{%
  ...
  how do I say that mathematics can be translated to declarative easily in declarative paradigm 
...
Putting all of the above together, Domain Specific Languages\endnote{%
  I have the feeling that you should introduce the concept of DSLs
  slightly higher.
  higher ???
}
...
The solution is to develop a programming language with a split personality, in our case, sometimes functional, sometimes logical and sometimes both.\eref{not-a-sentence}
...
feature set and the ability to mould itself according to the problem. A language with the ability to take the appropriate skill set and provide them to the programmer, which will reduce the hassle of jumping between languages or forcibly trying to solve a problem according to a paradigm.\eref{not-a-sentence}
...
This is the million dollar question.\endnote{%
  ``To dawn the avatar''?  This is poetic.  Did you mean ``to don''?
}

A \progLang{Haskell} with \progLang{Prolog} like features\endnote{%
