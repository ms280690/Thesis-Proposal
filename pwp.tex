...
first-order predicate logic. There are two types of clauses: \yyy{Facts}{facts} and rules.

\cite{website:prologintroumiami}
In \progLang{Prolog}
 all data objects are called terms\yyy{}{\Huge.}\endnote{%
   Fix.
}
 Atomic terms
 \yyy{Come}{come} in two forms\yyy{,}{:} atoms and integers.
 Atoms (this is a misnomeryyy{}{\Huge,}\endnote{%
   Fix
}
 as in logic predicates are called atoms\yyy{}{\Large,} and atoms are called constants. However, we'll stick to the \yyy{Prolog}{\progLang{Prolog}}\endnote{%
   Fix.
}
 convention.)
\yyy{}{consist of}
\yyy{Strings}{strings} of alphanumerics and ``\texttt{\_}'', starting with a lower case alphabetic.
Strings enclosed in 'single quotes'\endnote{%
  What?
}
Integers are numeric\yyy{}{\Large.}
Example
\par
...
where \yyy{Functor}{\Verb!functor!} starts with a \yyy{lower case}{lower-case} alphabetic. 
\paragraph{Example}
\par
\begin{minted}[linenos]{prolog}
...
Some functors are used in infix notation, e.g.\yyy{}{\Large,} \Verb!5+4!\yyy{}{\Large.}
Operators do not cause the associated function to be carried out.
\subparagraph{Variables}

Uppercase or `\Verb!_!' \xxx{for} start \xxx{of} variables\yyy{}{\Large.}
\paragraph{Example}
\begin{minted}[linenos]{prolog}
Who
What
...
Variables in \yyy{Prolog}{\progLang{Prolog}} are rather different \yyy{to}{from} those in most other languages. Further discussion and use is deferred until later.
