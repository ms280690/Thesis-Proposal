...
\progLang{Prolog} comes from the same family as \progLang{Haskell} i.e.
the declarative paradigm.
One of the reasons is that both \progLang{Haskell}, the base language and \progLang{Prolog}, the target language
...
\end{singlespace}
\caption{\progLang{Prolog} clause}
...
\item Scheduling policy:
  \endnote{\david{say what a scheduling policy is} \mehul{pg 93 art of prolog}}
  A scheduling policy defines how the additions and deletions of goals from the resolvent is performed by the interpreter.
  For instance, \progLang{Prolog} selects the leftmost goal and replaces the non deterministic choice of a clause by sequential search for a unifiable
  clause and backtracking.
  Stack scheduling policy, pops the topmost goal for reduction and pushes derived goals back.
...
\begin{code-list}[H]
  ...
  \end{singlespace}
  \caption{Tracing a simple Prolog computation \cite{Sterling:1994:APA:175753} : \prologConstruct{Clauses}}
