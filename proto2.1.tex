...
\yyy{}{\Huge Chapter}\ref{proto1}\endnote{%
...
have been taken from {\Huge \ref{proto1}\endnote{%
  Fix me!!
}}
...
The implementation consists of components that one would find in a Language Processing System \yyy{}{(see {\Huge
    ??}}~\ref{fig:A language-processing system}\yyy{,}{),}

\begin{figure}[th]
\centering
\includegraphics[scale = .95]{Language_Processing_System.png}
\caption{A language-processing system \yyy{}{(taken from} \cite{Aho:1986:CPT:6448}\yyy{}{)}}
\label{fig:A language-processing system}
\end{figure}

specifically speaking, parts of a compiler \yyy{}{(see {\Huge
    ??}}~\ref{fig:Phases of Compiler}\yyy{,}{).}
...
The architecture for a compiler as described in \ref{fig:Phases of Compiler} \yyy{would not be}{is not} needed
...
Beginning with language from Figure~\ref{tab:origgramp0201} is transformed into an isomorphically equivalent grammar described in 
...
After the language is opened up, the next step is to replace the current unification procedure to the one 
