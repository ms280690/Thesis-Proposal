...
implementation. The language modifications and unification mechanism have been taken from \ref{proto1} and adapted to fit in with the other
...
From the Figure~\ref{tab:origgramp0201} we will focus on the \textit{Term} since the others just add wrappers around expressions which can
...
\end{code-list}
A \textit{Unifier} is a list of \textit{Substitutions} which binds a variable to a value.  
...
The issue with the current approach is the rigidity of the procedure. Any language modification will cause a ripple effect across the 
