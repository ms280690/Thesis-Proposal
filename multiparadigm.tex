...
object-functional programming language which takes features from each of the two paradigms.\endnote{%
...
framework to work with different styles with a list of languages \cite{website:wikimpllist,website:dmoz} that suffices the requirements.\endnote{%
    Yet another metaphor that doesn't spin my wheels.\newline
    \mehul{changed}\newline
    \david{Now I just do not understand.}
}
Generally speaking, this\endnote{%
    What is ``it'' here?
    \mehul{changed}
}
is not very popular in programming circles; one reason could be that it\endnote{%
  What is ``it'' here?  Is this the same ``it'' as above?
}
is a very broad topic and specifying details can clear the confusion.\endnote{%
    Which fog?\newline
    \mehul{changed}\newline
...
give a brief overview of the aim and also the list of publications.\endnote{%
...
As as always since we are talking about \progLang{Haskell} we must\endnote{%
...
Although the language can do a lot; gaps do exist, such as the improvement of narrowing techniques.\endnote{%
...
\end{inparaenum}
have been included \yyy{}{below}, as the list is long and \xxx{is} not the main focus of the thesis.\endnote{%
