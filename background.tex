...
variables, lastly unifying all of the constraints giving the type of the result.\endnote{%
  ...
  \mehul{unificaiton is explained in proto1 and hints have been dropped at other places. Do you want me to write a short snipped here?}	
}
...
In this thesis we aim to merge two languages\endnote{%
  ``the two languages''?
...
The target language; \progLang{Prolog} is not an uncommon choice. To prove their power, many language have implemented \cite{swipembedd} a 
...
%The selection of languages is not uncommon and this not only the case with \progLang{Haskell,
\endnote{%
...
\cite{komorowski1982qlog,robinson1982loglisp,robinson1980loglisp}, \progLang{Java} \cite{wikiprolog, jlog}, 
\progLang{JavaScript} \cite{jscriptlog} and the list \cite{yieldprolog} goes on.\endnote{%
  This \P{} seems to repeat the preceding \P{}.  Merge them.
  \mehul{It is meant to be a replacement}
}
