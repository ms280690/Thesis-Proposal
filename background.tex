...
In the untyped $\lambda$ calculus functions have no predetermined \yyy{type whereas,}{type, whereas} typed lambda calculus puts
...
together and produce a result which \xxx{would} \yyy{exhibit}{exhibits} hybrid
properties.\endnote{%
  Make the suggested fixes in this sentence, then remove this \macroName{endnote}.
}
Further on we talk about more about the languages in question and approaches\xxx{\Verb[showspaces]! !}.\endnote{%
...
first language to incorporate monads \cite{wadler1992comprehending} for safe \unknownLabel{IO}\endnote{%
  Perhaps just use ``\textsl{input and output}'' in place of ``\textit{IO}''.
}\,. Monads can be 
...
It is as if the embedded language floats over the host\yyy{}{ language}.
...
mainstream.
One of the more successful attempts is \progLang{Scala}\yyy{}{\Verb[showspaces]! !}\cite{website:scala}. Simply speaking it is like a \textit{functional} 
