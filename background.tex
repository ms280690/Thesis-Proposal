...
They can be termed as multilingual run-times\endnote{%
  ...
  \begin{compactitem}
  \item \textit{run times}  \david{We do not use this one.}
  \item \textit{run-times}
  \item \textit{runtimes} \david{Let's pick this one, and use it consistently throughout.}
  \end{compactitem}
  is correct.
  What do basic O/S texts use?
  ``Multilingual'' is definitely one word.
  \path!https://en.wikipedia.org/wiki/Run_time_(program_lifecycle_phase)!
}
...
\mehul{changed}	\\ \david{Read it out loud.}
}
