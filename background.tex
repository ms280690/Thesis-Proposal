...
\cite{website:hdmtypesystemwiki} type system.\endnote{%
  ...
  \mehul{\cite{hindley1969principal}, \cite{milner1978theory}}
}
...
variables, lastly unifying all of the constraints giving the type of the result.\endnote{%
  ...
  \mehul{To explain the working, not directly required I guess but since \progLang{Haskell} is statically typed}
}
...
The same in \progLang{Haskell},\endnote{%
  \textcolor{dark-green}{Is the point here that templating helps, or what?}\newline
  \mehul{it is just one of the many ways to bring the languages together}\newline
  \textcolor{dark-green}{Say something in the text.}
}
...
The attempts fall into two categories as follows,\endnote{%
  See some other end-note or comment about commas before lists, {\Large And fix throughout}.
}
