...
described as composable computation descriptions \cite{website:monadshaskellorg} . Each \haskellConstruct{monad} consists of a 
...
\progLang{Prolog} seems to be the all time favourite for ``let's implement \progLang{Prolog} in the language X for
proving its power and expressibility''.
The \progLang{Prolog} language has been partially implemented \cite{swipembedd} in other 
...
using \progLang{Haskell} syntax and performing
