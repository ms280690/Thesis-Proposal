...
logic styles. Further on, we talk about the languages used for the implementations in this thesis, namely \progLang{Prolog} and 
\progLang{Haskell}. Moreover, this chapter provides a starting point for various approaches for bringing features of a different languages 
...
In this thesis we aim to merge two languages \progLang{Haskell} and \progLang{Prolog}
...
Monads\endnote{%
  ...
  \mehul{changed}\newline
  \david{No it isn't.  It's still marked \macroName{haskellConstruct}.}
	\mehul{changed}
}
can be 
...
The same in \progLang{Haskell} would be of the form:\endnote{%
  I've rewritten the \progLang{Haskell} code here.  Please check it.
	\mehul{looked at it breifly cannot find any mistake}
}
...
Listing~\ref{tab:prlgappndhskl} shows how a direct translation of Listing~\ref{fig:append-prolog} into \progLang{Haskell} while 
...
With the above in mind, coming back to the problem of implementing \progLang{Prolog} in \progLang{Haskell} and the possible 
methodologies to do so.\endnote{%
  Sentence fragment.\newline
  \mehul{changed}\newline
  \david{It's still a sentence fragment.}
}
...
\mehul{same as the last paragraph along with enumerations}
