...
The two most important ones in the family of declarative languages are the functional\eref{caps-choices} and
logical\eref{caps-choices} style of programming.

Functional programming\eref{caps-choices}, \cite{hughes1989functional} gets its name as the fundamental concept is
...
(\cite{hindley1969principal,milner1978theory,website:hdmtypesystemwiki}) type system.\endnote{%
  ...
  \mehul{Just different names.
  It is like Backus normal form and Panini-Backus normal form.}
...
\begin{enumerate}
%% TODO 2016-01-16.  The braces around Haskell below are suspicsion.
\item \progLang{Haskell}\endnote{%
  Inserted a missing \macroName{progLang}.
}
...
can be 
described as composable computation descriptions \cite{website:monadshaskellorg}.\endnote{%
...
one programming language can make use of services written in another programming language.\eref{not-a-sentence} For example, a function
...
They can be termed as multilingual run-times\endnote{%
  ...
  \mehul{I think all of them can be found}
}
...
modified at compile time allowing us to play with the code and going back and forth.\endnote{%
...
To accommodate the same it has to represented\endnote{%
  \textit{has to represented}: Do you mean ``\textsl{must be represented}''?
}
...
\cite{website:scala}.\endnote{%
  ...
  \mehul{Wrote something about \progLang{Scala} up there.}
}
