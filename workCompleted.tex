...
For a data type to be Unifiable, it must have instances of \markWord{Functor}, \markWord{Foldable} and \unknownLabel{Traversable}\endnote{%
  Mark the preceding word or phrase with a macro indicating its kind.
}\elabel{text-kind}
.
...
After infinitely many iterations we should get to a fixed point where further iterations make no difference. It means that applying one more 
\markWord{ExprF} would not change anything --\endnote{%
  the preceding should be an em-dash, not an en-dash.
}
a fixed point does not move under \markWord{ExprF}. It's like adding one to infinity: you get back infinity.   

\item[How do that in \progLang{Haskell}]
In \progLang{Haskell}, we can express the fixed point of a type constructor f as a type:
...
\mint{haskell}|Fix ExprF|\yyy{}{\Huge .}
...
\begin{minted}[linenos]{haskell}
 Fix f = f (Fix f)
\end{minted}
\textcolor{blue}{The Essence of Algebra}\endnote{%
  Is this supposed to be a \macroName{paragraph} or \macroName{subsection} or something?
}
...
Since here we are dealing with types, we have to\endnote{%
  Use ``must'' in place of ``have to'' where you can.
  \mehul{changed}
}\elabel{must-have-to} define a type function, otherwise known as type
constructor.
...
a fixed point doesn't move under \markWord{ExprF}.
It's like adding one to infinity: you get back infinity.

In \progLang{Haskell}, we can express the fixed point of a type constructor \markWord{f} as a type:
