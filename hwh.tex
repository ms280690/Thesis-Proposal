...
In this chapter we discuss the properties of \progLang{Haskell}\yyy{}{\LARGE.}\endnote{%
  Fix and remove this footnote.
}

This chapter discusses the properties of the host language \progLang{Haskell} and mainly the feature set it
provides for \yyy{embedding domain specific languages(EDSLs)}{embedded
  domain specific languages (eDSLs)}.\endnote{%
  Be consistent with previous notation, and put a space before the open ``\textbf{(}''.
}

\david{%
  This chapter needs to be rewritten without the list.
}
...
}\elabel{such-as-like} variables, control structures and 
  arithmetic types.
...
Functional Programming Languages\endnote{%
