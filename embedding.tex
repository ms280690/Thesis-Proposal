...
\progLang{Prolog} to the host language in terms of types and modules among others.\endnote{%
...
still is an unpublished implementation\endnote{%
...
This seems to be a common problem with the other\endnote{%
...
enables encoding of search problems.\endnote{%
  ...
  Attempting to do AI like search stuff in \progLang{Haskell}
}
...
\subsection{Prolog Libraries}
  ...
  To replicate \progLang{Prolog} like\endnote{%
  ...
  powerful but is an foreign function interface so it is ``\progLang{Prolog} in \progLang{Haskell}'' but we need Prolog
...
\subsection{Logic Libraries}
  ...
  Prolog is heavily based on formal logic.\endnote{%
    comma splice.
}
...
I do not know since we are just listing stuff out.
}
