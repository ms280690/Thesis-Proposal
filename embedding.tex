...
with, but still lacks complete list support\endnote{%
  ...
  \mehul{I forgot}.
  \mehul{i remembered. All the different types of data types in \progLang{Prolog} are uni type just like our abstract syntax language.}
}
...
The purely declarative nature lacks basic requirements such as support for \languageConstruct{modules}.\endnote{%
  ...
  declarative; or \dots?
  \mehul{I do not know about the paradigm, but \progLang{Prolog} language having a declarative nature lacks modules}
}
...
enables encoding of search problems.\eref{not-a-sentence} \mehul{changed}

\section{Perhaps the section title should be ``Books related to embedding Programming Languges''}
...
\section{Perhaps the section title should be ``Papers related to embedding Programming Languges''}
...
solutions are instances.\eref{sentence-fragment},\endnote{%
...
capabilities \progLang{Haskell} seems to be a popular choice\endnote{%
  Perhaps its the sentence order rather than the metaphor that is the problem here.
  Does
  \vspace*{-1\baselineskip}
  \begin{quote}\slshape
    With a host of related libraries \progLang{Haskell}
    seems to be already in the race
    \textbf{\upshape (or:} seems to be well-positioned\textup{)}
    to replicate \progLang{Prolog}-like capabilities.
  \end{quote}
}
