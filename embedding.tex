...
in \progLang{Haskell} for \haskellConstruct{Hugs 98}, called \haskellConstruct{Mini} \progLang{Prolog} \cite{website:mini-prolog-hugs98}.\endnote{%
  Reword.
  ...
  \item
    ...
    leads us something.
  \item
    \texttt{Mini \progLang{Prolog}} (\cite{website:mini-prolog-hugs98}) is
...
enables encoding of search problems.\eref{not-a-sentence}\textsuperscript{\,}\endnote{%
...
itself, a few exist \cite{nanoprolog-lib} being a preliminary or
``mini \progLang{Prolog}'' as such with not much in it to be able to be useful, \cite{hswip-lib} is all powerful but is an
foreign function interface so it is ``\progLang{Prolog} in \progLang{Haskell}'' but we need \progLang{Prolog} for it,
...
\subsection{Logic Libraries}
  ...
  The next category is about the logical aspects of \progLang{Prolog}; again a handful of libraries do exist and provide a
...
\cite{cmu-lib} that unify two \progLang{Prolog} terms and return the resulting
substitution.\endnote{% 
  Check for unmarked \texttt{Prolog}s and strange capitals in the middle of sentences.
}
