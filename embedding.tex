...
Although this implementation takes as reference the working of the \progLang{ Prolog} Engine\endnote{%
  \david{What does the \cite{website:mini-prolog-hugs98} implementation takes as reference?
    ...
    \mehul{I checked the website, could not find anything as such.}
  }
}
...
with, but still lacks complete list support\endnote{%
  ...
  \david{Are you saying that you want lists in \progLang{Prolog} to be constructed from something other than
    ...
    \mehul{We discussd this, waiting for feedback}
}
...
Some \progLang{Prolog} implementations lack basic requirements such as support for \languageConstruct{modules}.
...
potential users question the choice of \progLang{Prolog}, when \progLang{Haskell}, also a language in the family of declarative languages 
is so popular.\endnote{%
...
As \progLang{Haskell} is relatively new in terms of being popular, other functional languages such as \progLang{Scheme} have
explored the problem of embedding in great depth. \cite{friedman05reasoned} aims at adding a few
constructs to \progLang{Scheme} to bring together both styles of declarative programming and capture the essence of
\progLang{Prolog} in the host language.
Moreover, \progLang{Haskell} also claims for it to be suitable for basic logic programming using the
...
\cite{krishnamurthi2007programming} to popularize the idea of embedding \progLang{Prolog}.\endnote{%
...
to the other.
While other publications such as \cite{erwig2004escape} attempt at exercising \progLang{Haskell} features without adding any new 
constructs.\endnote{%
    This is a sentence fragment.
  }\elabel{sentence-fragment} \mehul{changed}
It uses \progLang{Haskell}'s type class to express a general structure for the problem while its instances represent the solutions. 
\eref{sentence-fragment}\textsuperscript{,}\endnote{%
