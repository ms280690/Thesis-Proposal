...
\markWord{cut} and \markWord{fail} with higher order functions.
...
use of the \progLang{Haskell} \markWord{State Monad} to capture and maintain a global state.
...
\subsection{Logic Libraries}
  ...
  \cite{logict-lib} is a continuation-based, backtracking, logic programming monad which attempts to replicate\endnote{%
...
But the difference between an ordinary library and an eDSl\endnote{%
  Here ``eDSl'' should be ``eDSL''.
}
is the feature set provided and the degree of embedding
