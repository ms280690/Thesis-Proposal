...
in solving the problems in the domain. This chapter mainly reviews the content related to embedding \progLang{Prolog} in
...
Before moving on to the formal content such as publications, modules and libraries\yyy{}{\Large,} let's take a look at some of the
...
\cite{website:lambda-the-ultimate} lists a number of \progLang{Prolog} implementations in a variety of
...
in \progLang{Haskell} for \haskellConstruct{Hugs 98}, called
\haskellConstruct{Mini} \progLang{Prolog} \cite{website:mini-prolog-hugs98}.
...
family of Declarative Languages.\eref{caps-choices}\textsuperscript{,}\endnote{%
...
enables encoding of search problems\yyy{}{in \progLang{Haskell} rather than \progLang{Prolog}}.\eref{not-a-sentence}\textsuperscript{,}\endnote{%
...
As \progLang{Haskell} is relatively new in terms of being popular, its predecessors such as\endnote{%
...
\progLang{Prolog}.
Moreover, \progLang{Haskell} also claims for it to be suitable for basic logic programming naturally using the \haskellConstruct{List
Monad} \cite{website:logicprogexamplehaskell}.
A general \yyy{out look}{outlook} towards implementing \progLang{Prolog} has also been discussed by
...
Abundant literature
can be found on embedding detailed parts of \progLang{Prolog} features
such as\eref{such-as-like} basic constructs, search strategies and data types.
...
\progLang{Prolog}-like\eref{language-like} working not a multiparadigm
declarative language.
\progLang{Prolog} \prologConstruct{predicates} are translated to \progLang{Haskell} functions which produce a stream of results
lazily depicting depth first search with support for different strategies and practical operators such as
\prologConstruct{cut} and \prologConstruct{fail} with higher order functions.
...
This results in typed logical predicates and a backtracking \haskellConstruct{monad} with support for various data types and search
strategies.
...
The main contributions are a Backtracking Monad Transformer that can enrich any \haskellConstruct{monad} with backtracking abilities
and a \haskellConstruct{monadic encapsulation} to turn a \progLang{Prolog} predicate into a \progLang{Haskell} function.
...
``mini \progLang{Prolog}'' as such with not much in it to be able to be useful,\eref{comma-splice} \cite{hswip-lib} is all powerful but is \yyy{an}{a}
...
\cite{prolog-lib}\yyy{}{\Large,} which is the only implementation that comes the closest\endnote{%
  Here ``\textit{only}'' fights with ``\textit{closest}''.
  Use either
  \begin{quote}
    \cite{prolog-lib}, which is the only implementation that comes \xxx{the} \yyy{closest}{close}\dots
  \end{quote}
  or
  \begin{quote}
    \cite{prolog-lib}, which is the \xxx{only} implementation that comes the closest\dots
  \end{quote}
}
...
no \haskellConstruct{monadic} support and an REPL without the ability to ``write a \progLang{Prolog} Program File''.\eref{caps-choices}

%\item[$\bullet$]Logic Libraries
\subsection{Logic Libraries}

The next category is about the logical aspects of \progLang{Prolog}; again a handful of libraries \xxx{do exist
  and} provide \xxx{a 
part of the} functionality \xxx{which are} related
  ...
  \cite{logict-lib} is a continuation-based, backtracking, logic programming \haskellConstruct{monad} which attempts to replicate\endnote{%
...
There are two libraries \xxx{that exist}
\xxx{\cite{unification-fd-lib}}
\yyy{, \cite{cmu-lib}}{(\cite{unification-fd-lib}, \cite{cmu-lib})} that unify two \progLang{Prolog} terms and return the resulting
substitution.\endnote{% 
  Make the changes suggested or similar, then delete this endnote.
}
