...
Although the host language \yyy{}{\progLang{Haskell}} has distinct characteristics such as lazy evaluation and a strong type system, the proposed
...
Though not very efficient \yyy{nor}{or} practical,
...
There are two libraries (\cite{cmu-lib,unification-fd-lib}) that unify two \progLang{Prolog} terms and return
the resulting substitution.\endnote{%
  This \P{} makes it sound like unification is unique to \progLang{Prolog} and that the two libraries are
  \progLang{Prolog} specific, which at least in the case of \cite{unification-fd-lib} is incorrect.
}
