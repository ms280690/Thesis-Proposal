...
The above approach allows to us to jump between the environments (grammars) easily provided the back and forth
conversion capabilities Listing~\ref{tab:prlg0201monadicunificationtranslation} and
...
\item{monadicUnification}
  ...
  in a \languageConstruct{unifier}.
  The type signature of \haskellConstruct{unify} is shown in Listing~\ref{tab:unifytypesignature}.

\begin{code-list}[H]
	\begin{singlespace}
\inputminted{haskell}{haskell-proto2-unify-type-signature.hs}
	\end{singlespace}
\caption{\codeLibrary{unification-fd} \haskellConstruct{unify} type signature}
\label{tab:unifytypesignature}
\end{code-list}

We also return a union of the variable dictionaries for each of the terms required to convert the result into the
eDSL.
